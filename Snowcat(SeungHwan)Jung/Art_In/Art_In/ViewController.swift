...
class first: UITabBarController {
...
class WebScene: CHWebViewController {
    @IBInspectable var pageURL:String = "www.google.co.kr/search?q=%EA%B0%95%EC%95%84%EC%A7%80&rlz=1C5CHFA_enKR583KR583&source=lnms&tbm=isch&sa=X&ved=0ahUKEwj00NGH29LJAhWBNKYKHQDlA5EQ_AUIBygB&biw=1152&bih=885"
...
public extension UIViewController {
    ...
    @IBAction func modalDismiss_noAni(sender : AnyObject){
        self.dismissViewControllerAnimated(false, completion: nil)
    }
    
    
    @IBAction func modalDismTwo_noAni(sender : AnyObject){
        self.dismissViewControllerAnimated(false, completion: nil)
    }
    ...
    @IBAction func navigationBack_noAni(sender : AnyObject){
        if let navigationCongtroller = self.navigationController {
            navigationController?.popViewControllerAnimated(false)
        }
    }
    @IBAction func navigationTwoBack_noAni(sender : AnyObject){
        if let navigationCongtroller = self.navigationController {
            navigationController?.popViewControllerAnimated(false)
    ...
    @IBAction func navigationBackToRoot_noAni(sender : AnyObject){
        if let navigationCongtroller = self.navigationController {
